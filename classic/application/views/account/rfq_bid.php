...
<td>
<div class="btn-group">
  <button type="button" value="or" id="animateinitme" class="addbidme btn btn-default btn-warning" >Bid</button>
</div>
</td>
<td>
<input type="text" style="width:40px;border:1px solid #'.$bc.'" '.'value="'.htmlspecialchars($value->serialno).'" name="rfqtsn[]" readonly />
</td>
<td>
<input type="text" '.'value="'.htmlspecialchars($value->cdesc).'" name="rfqtcdesc[]" readonly />
</td>
<td>
<input type="text" style="width:80px" '.'value="'.htmlspecialchars($value->spec).'" name="rfqtspec[]" readonly />
</td>

<td>
<input type="text" style="width:80px" '.'value="'.htmlspecialchars($value->dimension).'" name="rfqtdi[]"  readonly  />
</td>

<td>
<input type="text" style="width:80px" '.'value="'.htmlspecialchars($value->quantity).'" name="rfqtqu[]"  readonly />
</td>

<td>
<select style="width:80px" name="rfqtuom[]"  readonly >
'.populate_uom($value->uom).'
</select>
</td>
<td>
<input type="text" style="width:40px" onblur="rfqtotal()" '.'value="'.htmlspecialchars($value->reqprice).'" name="rfqtreqprice[]"  readonly />
</td>
<td>
<input type="text" style="width:40px" '.'value="'.htmlspecialchars($value->reqleadtime).'" name="rfqtleadtime[]"  readonly />
<input type="hidden" style="width:40px" value="1" name="isorig[]"/>
...
<td>
<input type="text" '.'value="'.htmlspecialchars($value->cdesc).'" name="rfqtcdesc[]" />
</td>
<td>
<input type="text" style="width:80px" '.'value="'.htmlspecialchars($value->spec).'" name="rfqtspec[]" />
</td>

<td>
<input type="text" style="width:80px" '.'value="'.htmlspecialchars($value->dimension).'" name="rfqtdi[]" />
</td>

<td>
<input type="text" style="width:80px" '.'value="'.htmlspecialchars($value->quantity).'" name="rfqtqu[]" />
</td>
...
<td>
<input type="text" style="width:40px" onblur="rfqtotal()" '.'value="'.htmlspecialchars($value->reqprice).'" name="rfqtreqprice[]" />
</td>
<td>
<input type="text" style="width:40px" '.'value="'.htmlspecialchars($value->reqleadtime).'" name="rfqtleadtime[]" />
</td>
<td>
</td>
</tr>';
...
<b><?php echo dp($info_holder,"RFQ Bidding");?></b>
</div>
</div>
<div id="maincon">
		...
		<div class="row" >
			<div class="col-xs-3">
				<div class="offer offer-default">
					...
					<li><b><?php echo dp($info_holder,"Upload only necessary documents");?></b></li>
					
					<li><b><?php echo dp($info_holder,"You can update the information which are not readonly");?></b></li>
					</div>
					</div>
				</div>
			</div>
...
<div id="rfqcontent" style="margin-left:10px;">
<form id="rfqformdata" action="../../rfq/rfq_bid_submit" method="post" enctype="multipart/form-data">
<table style="width:100%" id="rfqmaindata">
<tr>
...
<td>
<input type="text" name="rfqtitle"  id="rfqtitle" value="<?php echo $rfqm[0]->rfq_title;?>" required="required" readonly/>
</td>
...
<td>
<input type="text" name="rfqissuedby"  id="rfqissuedby" value="<?php echo $rfqm[0]->rfq_issueby;?>" required="required" readonly/>
</td>
...
<td>
<input type="text" name="rfq_issuedate"  id="rfq_issuedate" value="<?php echo $rfqm[0]->rfq_issue_date;?>" required="required" readonly/>
...
<td>
<input type="text" name="rfq_closedate"  id="rfq_closedate" value="<?php echo $rfqm[0]->rfq_close_date;?>" required="required" readonly/>
...
<td>
<input type="text" name="rfqcimports"  id="rfqcimports" value="<?php echo $rfqm[0]->rfq_country_imports;?>" required="required" readonly/>
...
<td>
<input type="text" name="rfqcexports"  id="rfqcexports" data-items="4" data-provide="typeahead" value="<?php echo $rfqm[0]->rfq_pref_cn_export;?>" required="required"/>
</td>
...
<div>
<div id="rfqinit" style="margin-top:20px;width:100%;min-width:500px;">
<div style="border:3px solid whitesmoke">
<p style="font-weight:bold;font-size:15px;padding-left:30px;height:20px;background-color:whitesmoke">RFQ information</p>

<table class="table" style="width:100%" id="rfqtable">
<thead>
<tr>
<th>
<?php echo dp($info_holder,"Bid Action");?>
</th>
<th>
...
<div style="text-align:right;width:80%;margin-bottom:20px;margin-top:10px">
<div style="border:3px solid whitesmoke">
<p style="font-weight:bold;font-size:15px;padding-left:30px;height:20px;background-color:whitesmoke;text-align:left"><?php echo dp($info_holder,"RFQ Attached Documents");?></p>
<thead>
<table class="table" style="width:100%" id="rfqtableadded">
<th>
...
<?php
 ...
 echo populate_docs($rfqd,$info_holder);?>
</tbody>
</table>
...
<div style="border:3px solid whitesmoke;width:500px;margin-top:10px;margin-bottom:10px;">
<p style="font-weight:bold;font-size:15px;padding-left:30px;height:20px;background-color:whitesmoke"><?php echo dp($info_holder,"Bidder Supporting Documents");?></p>
...
<input type="submit" class="btn" value="<?php echo dp($info_holder,"Submit Bid");?>">
...
var thisRow = $("#rfqtable tbody tr").eq(rowIndex);
var cc=$( thisRow ).clone();
$("#rfqtable tbody tr").eq(rowIndex).after(cc);

var newRow = $("#rfqtable tbody tr").eq(rowIndex+1);
...
$( newRow ).find( 'button').after('<button type="button" class="btn btn-danger" onclick="rme(this);">Remove</button>')
...
$( "#rfqformdata" ).submit(function( event ) {
$("#myModal").modal();
return;
});
$('#animateinitme').focus();
function testAnim(x) {
    $('#animateinitme').removeClass().addClass(x + ' animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
      $(this).removeClass();
	  $(this).addClass("addbidme btn btn-default btn-warning");
    });
  };
testAnim("flash");  
</script>